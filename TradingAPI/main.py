...
class StartRun(Resource):
    ...
    def get(self):
        ...
        response = jsonify(states=[state.serialize() for state in trad.env.States], opens=his['Open'].tolist(), dates=his.reset_index().iloc[:, 0].tolist(), macd=macds, signals=signals)
        response.headers.add("Access-Control-Allow-Origin", "*")
        return response
